...
/**
 * This function is to append order to manifest
 * @param {number} orderlineId  (In Array) 
 * @param {number} manifestSourceId
 */
...
/**
 * Check Business rule 1: BR_PLN_1
 */
...
/**
 * Check Business rule 2: BR_PLN_2
 */
...
/**
 * Check Business rule 3: BR_PLN_3
 */
        ...
        client.search({
            ...
            if (result.hits.hits.length) {
                ...
                client.search({
                    ...
                    if (manifestResult.hits.hits.length) {
                        if (!([null, undefined, ''].includes(manifestResult.hits.hits[0]._source.freightType))) {
                            ...
                            sequlizeConn.getMySQLConn().then((conn) => {
                                return conn.transaction().then(function (t) {
                                    return manifestSchema(conn).update({
                                        ...
                                        where: {
                                            manifestId: {
                                                [Op.eq]: manifestResult.hits.hits[0]._source.id
                                            }
                                        }
                                    }, {
                                        transaction: t
